    ...
    async def call_api(
        ...
        try:
            async with session.post(
                url=request_url, headers=request_header, json=self.request_json["body"]
            ) as response:
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                data = (
                    [self.task_id, self.request_json, [str(e) for e in self.result], self.metadata]
                    if self.metadata
                    else [self.task_id, self.request_json, [str(e) for e in self.result]]
                )
            ...
            data = (
                [self.task_id, self.request_json, response, self.metadata]
                if self.metadata
                else [self.task_id,self.request_json, response]
            )
            append_to_jsonl(data, save_filepath)
            status_tracker.num_tasks_in_progress -= 1
...
def api_endpoint_from_url(request_url):
    ...
    match = re.search("^http://[^/]+/v\\d+/(.+)$", request_url)
    if match is None:
        ...
        if match is None:
            ...
            if match is None:
                match=re.search("https://api.deepseek.com/chat/completions", request_url)
                if match is None:
                    match=re.search("https://openrouter.ai/api/v1/chat/completions", request_url)
                    if match is None:
                        match=re.search("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", request_url)
                        if match is None:
                            raise ValueError(f"Could not extract endpoint from URL {request_url}")
    return match[1]
...
def num_tokens_consumed_from_request(
    ...
    num_tokens = 0
    for message in request_json["body"]["messages"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_requests_per_minute", type=int, default=1_000 * 0.5)
